package mchorse.bbs_mod.camera.clips.screen.nodes;

import mchorse.bbs_mod.forms.forms.shape.nodes.ShapeNode;

import java.util.Arrays;
import java.util.Collections;
import java.util.List;

/**
 * Standalone vignette effect node. No output connections needed — the evaluator
 * detects this node in the graph and applies its inputs directly to the effect.
 *
 * Inputs:
 *   0  strength    0=off, 1=full black edges
 *   1  smoothness  0…1 transition softness
 *   2  color       ARGB packed color (default opaque black)
 */
public class VignetteEffectNode extends ShapeNode
{
    private static final List<String> INPUTS  = Arrays.asList("strength", "smoothness", "color");
    private static final List<String> OUTPUTS = Collections.singletonList("layer");

    @Override
    public String getType()
    {
        return "screen_vignette";
    }

    @Override
    public List<String> getInputs()
    {
        return INPUTS;
    }

    @Override
    public List<String> getOutputs()
    {
        return OUTPUTS;
    }
}
